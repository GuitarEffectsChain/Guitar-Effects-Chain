...
public class Server {
	...
	final int PUERTO = Integer.parseInt(Config_frame.connection_port_box.getSelectedItem().toString());// Puerto que utilizara el servidor utilizar este
							// mismo en el cliente
	String IP_client;
	String mdata;
	...
	private void setAllSlidersComp(final String[] data) {
		...
		if (SwingUtilities.isEventDispatchThread()) {
			...
			boolean aux = Integer.parseInt(data[7]) == 1 ? true : false;
			
			if(aux != Main_frame.compressor_button.isSelected()){
				Main_frame.compressor_button.doClick();	
			}
	...
	private void setAllSlidersReverb(final String[] data) {
		...
		if (SwingUtilities.isEventDispatchThread()) {
			boolean aux = Integer.parseInt(data[16]) == 1 ? true : false;
			
			if(aux != Main_frame.reverb_button.isSelected()){
				Main_frame.reverb_button.doClick();	
			}
			} else {
		    SwingUtilities.invokeLater(new Runnable () {
		        public void run () {
					...
					Main_frame.pr_name = data[0];
					Main_frame.preset_name.setText(data[0]);
					}
		    });
		}
	}
	...
	private void setAllSlidersDelay(final String[] data) {
		...
		boolean aux = Integer.parseInt(data[12]) == 1 ? true : false;
		
		if(aux != Main_frame.delay_button.isSelected()){
			Main_frame.delay_button.doClick();	
		}
			}		
	...
	private void setSliderReverbParam(String[] param) {
		...
		case 2:
			...
			break;
		case 3:
			boolean aux = Integer.parseInt(param[2]) == 1 ? true : false;
			if(aux != Main_frame.reverb_button.isSelected()){
				Main_frame.reverb_button.doClick();	
			}
			break;
	...
	private void setSliderAmpParam(String[] param) {
		...
		case 4:
			...
			break;		
		case 5:
			Main_frame.amp_volume_slider.setValue(Integer.parseInt(param[2]));
			break;	
		case 6:
